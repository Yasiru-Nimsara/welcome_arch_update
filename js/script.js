...
big_loader.addEventListener("click", () => {
...
function requestTypes(startPoint, endPoint) {
  ...
  else if (startPoint.lat <= ivorStatueLat && endPoint.lat >= ivorStatueLat) {
    return {
      origin: startPoint,
      destination: endPoint,
      travelMode: google.maps.TravelMode.WALKING,
...
function drawRoadRoute(id, start, end, routeColor) {
  ...
  const request = requestTypes(start, end);
  ...
  directionsService.route(request, (result, status) => {
    ...
    big_loader.style.display = "none";
    if (!drawRoute || id !== "RouteQR") {
      warning.style.display = "flex";
      streetview_close.style.display = "none";
      _3d_close.style.display = "none";
      more_cont.style.display = "none";
      warning_link = `https://www.google.com/maps/dir/?api=1&destination=${endPlace.lat},${endPlace.lng}&travelmode=driving`;
    }
  });
}
...
warning_close.addEventListener("click", () => {
  ...
  streetview_close.style.display = "block";
  _3d_close.style.display = "block";
});
...
function locateUser() {
  ...
  try {
    if (!currentLat || !currentLng) {
      ...
      const { lat, lng } = get_user_current_location();
      pos = { lat: lat, lng: lng };
    } else {
...
function navigate_QR(startPoint, endPoint) {
  ...
  startInfo.open({ anchor: startMarker, map });
...
function navigate_auto() {
  ...
  autoNavInterval = setInterval(() => {
    ...
    if (!lastPos || Math.abs(lastPos.lat - pos.lat) > 0.00001 || Math.abs(lastPos.lng - pos.lng) > 0.00001) {
      clearRoute("RouteAUTO");
